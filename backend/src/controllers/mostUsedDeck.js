const findMostUsedDeck = (battles) => {
    ...
    battles.forEach(battle => {
      if (battle.team && battle.team.length > 0) {
        ...
        const deck = battle.team[0].cards.map(card => card.name).sort().join(', ');
        const elixirSum = battle.team[0].cards.reduce((acc, card) => acc + card.elixirCost, 0);
    ...
    const sortedDecks = Object.entries(deckUsage).sort((a, b) => b[1].count - a[1].count);
  
    //Joueur joue avec un seul deck pour tous les matchs
    if (sortedDecks.length === 1) {
      return { mostUsedDeck: sortedDecks[0][0], elixir: sortedDecks[0][1].elixir };
    }
  
    //Le joueur joue un deck différent pour chaque match
    if (sortedDecks.length > 0 && sortedDecks[0][1].count === 1 && sortedDecks.length === battles.length) {
      return { message: "Pas de deck le + utilisé pour ce joueur, svp essayez avec un autre joueur" };
    }
  
    //Il existe plus qu'un deck le plus utilisé
    const mostUsedDecks = sortedDecks.filter(deck => deck[1].count === sortedDecks[0][1].count);
    if (mostUsedDecks.length === 1) {
      return { mostUsedDeck: mostUsedDecks[0][0], elixir: mostUsedDecks[0][1].elixir };
    }
