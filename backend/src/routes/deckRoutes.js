...
// Route POST pour obtenir les données des 25 dernières batailles d'un joueur
router.post('/getBattleData', async (req, res) => {
  ...
  try {
    ...
    const battleData = battles.map(battle => ({
      ...
      opponentDeck: battle.opponent[0].cards.map(card => card.name).join(', '),
      playerElixirCost: (battle.team[0].cards.reduce((acc, card) => acc + card.elixirCost, 0) / battle.team[0].cards.length).toFixed(2),
      opponentElixirCost: (battle.opponent[0].cards.reduce((acc, card) => acc + card.elixirCost, 0) / battle.opponent[0].cards.length).toFixed(2),
      ...
      opponentPrincessTowersHP: battle.opponent[0].princessTowersHitPoints.join(', '),
    }));
...
router.post('/getMostUsedDeck', async (req, res) => {
  const playerTag = req.body.playertag;
  try {
